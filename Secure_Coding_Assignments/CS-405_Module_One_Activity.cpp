...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
	...
	{
		// use try catch to return an error without breaking the program at runtime
		try {
			// check if result is greater than maximum value possible before incrementing
			// numeric_limits<T>::max() returns the largest finite value of the given type
			if (result > std::numeric_limits<T>::max() - increment) {
				...
				// increment the result otherwise
				result += increment;
			}
		}
		// catch the overflow error when thrown
		catch (const std::exception& e) {
			// log to console the string identifying exception
			std::cerr << e.what();
		}
	}
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps)
{
	...
	for (unsigned long int i = 0; i < steps; ++i)
	{
		// use try catch to return an error without breaking the program at runtime
		try {
			// Detect when overflow will happen 
			// check if result is less than minimum value possible before decrementing
			// numeric_limits<T>::min() returns the smallest finite value of the given type
			if (result < std::numeric_limits<T>::min() + decrement) {
				...
				// decrement the result otherwise
				result -= decrement;
			}
		}
		// catch the overflow error when thrown
		catch (const std::exception& e) {
			// Return error message to tell test the addition failed
			// log to console the string identifying exception
			std::cerr << e.what();
		}
	}
	// return correct value when no overflow happened
	return result;
}
...
void test_overflow()
{
	...
	// Detect when add_numbers failed
	catch (const std::overflow_error& e) {
		// Inform user the overflow happened
...
void test_underflow()
{
	...
	// Detect when subtract_numbers failed
	catch (const std::overflow_error& e) {
		// Inform user the overflow happened
		std::cerr << e.what() << std::endl;
	}
