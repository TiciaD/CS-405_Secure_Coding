...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    ...
    // Hold string version of sql query
    std::string sql_string = sql.c_str();
    // Find "or" substring in sql query
    // will either hold index of where substring is found or npos
    size_t OR_found = sql_string.find("or");

    // if "or" substring found and therefore has an index
    if (OR_found != std::string::npos) {
        // Find "=" substring in sql query starting after "or" substring
        size_t equals_found = sql_string.find("=", OR_found + 1);
        // if "=" substring found after "or" substring
        if (equals_found != std::string::npos) {
            // Print error message
            std::cout << "ERROR - Possible SQL Injection Found: " << sql.c_str() << std::endl;
            // return false and break from function
            return false;
        }
    }
