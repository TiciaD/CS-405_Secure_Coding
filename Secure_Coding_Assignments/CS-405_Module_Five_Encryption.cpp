...
std::string read_file(const std::string& filename)
{
    ...
    // declare ifstream variable to hold file
    std::ifstream input_data_file(filename);

    // initialize string stream variable
    std::stringstream buffer;

    // read file and transfer contents to buffer variable
    buffer << input_data_file.rdbuf(); 

    // convert buffer to string and overwrite file_text variable with actual file contents
...
void save_data_file(const std::string& filename, const std::string& student_name, const std::string& key, const std::string& data)
{
    ...
    if (output_file.is_open()) // check if file is open
    {
        //  file format
        //  Line 1: student name
        ...
        // Get current timestamp and write to file
